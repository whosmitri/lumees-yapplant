...
void setup() {
  ...
  Serial.println("\n[Lumees Yapplant] aqui!! Inicializando o Hardware...");
  ...
  dht.begin();
  Serial.println("Sensor DHT11 (Climático/Ar) inicializado!");
...
void loop() {
  // nada aqui por causa do modo sonequinha !!
}

// função que faz leitura dos sensores
void leituras() {
  ...
  //// validação de leituras caso o DHT falhe
...
// função que liga e configura o wifi
void ligarWifi() {
  Serial.println("Sensores lidos! Ligando Wi-Fi para envio...");

  // 1. WIFIMANAGER (conexão externa)
  WiFiManager wifiMan;

  // reduzir o timeout do WiFiManager economiza bateria se a rede cair
  wifiMan.setConfigPortalTimeout(100);
  ...
  if(!wifiMan.autoConnect("Lumees_Yapp_Setup", "lumees024")) {
    Serial.println("Falha na conexão e estourou o tempo limite. Indo dormir para não gastar bateria...");
    modoSonequinha();
  }
...
// função que mostra os resultados/dados na tela em Serial Monitor
...
void enviarDados() {
  // 1. EMPACOTANDO TUDO NO JSON COMPATÍVEL COM API
  ...
  // 2. DISPARANDO O POST HTTP PARA O BACKEND
...
// função que ativa o Deep Sleep de forma limpa
void modoSonequinha() {
  Serial.println("Cortando energia dos sensores e desconectando o Wi-Fi para dormir de forma segura...");
  
  // corta energia dos sensores
  digitalWrite(PIN_POWER_SENSORES, LOW);
  // desconecta o wifi
  WiFi.disconnect(true);
  // força o rádio a desligar completamente
  WiFi.mode(WIFI_OFF);
